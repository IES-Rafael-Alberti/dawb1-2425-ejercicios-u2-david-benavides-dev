...
def pedir_libros(libro = None) -> str:
    """
    ...
    while libro != "*":
        libro = input("Libro: ")
        if libro != "/":
            digitos_en_libro = contar_numeros(libro)
        elif libro == "/":
            print(f"Linea completa. Aparecen {digitos_en_libro} dígitos numéricos.")
            contar_libro += 1
    return f"Fin. Se leyeron {contar_libro} lineas completas."
